...
function appendUtmToYoutubeLinks(text, pagePath) {
  if (typeof text !== 'string') return text;
  
  // Ищем маркдаун ссылки [текст](url), где url это youtube.com или youtu.be
  return text.replace(/\[([^\]]+)\]\((https?:\/\/(?:www\.)?(?:youtube\.com|youtu\.be)[^\s)]+)\)/g, (match, linkText, url) => {
    try {
      const urlObj = new URL(url);
      urlObj.searchParams.set('utm_source', 'aadna.ru');
      
      let slug = pagePath.replace(/\/$/, '');
      if (!slug.startsWith('/')) slug = '/' + slug;
      if (slug === '/') slug = '/home';
      
      urlObj.searchParams.set('utm_medium', slug);
      urlObj.searchParams.set('utm_campaign', 'youtube_outbound');
...
function processExtraText(obj, pagePath) {
  if (!obj || typeof obj !== 'object') return;
  for (const key of Object.keys(obj)) {
    if (typeof obj[key] === 'string') {
      obj[key] = appendUtmToYoutubeLinks(obj[key], pagePath);
    } else if (typeof obj[key] === 'object') {
      processExtraText(obj[key], pagePath);
    }
  }
}
...
export function normalizeAadnaContent(content, existingContent, defaultSlug = '') {
  ...
  // Автоматически добавляем UTM метки для всех текстовых полей, содержащих ссылки на YouTube
  if (normalizedContent.extra) {
    processExtraText(normalizedContent.extra, normalizedContent.path);
  }
